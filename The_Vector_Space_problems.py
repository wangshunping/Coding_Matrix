...
def vec_select(veclist, k):
    ...
    True
    '''
    tmp = [x for x in veclist if x[k] == 0]
    for x in tmp:
        if k in x.f:
            x.f.pop(k)
    return tmp
...
def scale_vecs(vecdict):
    ...
    [True, True]
    '''
    return {vecdict[x]/x for x in vecdict}
...
def GF2_span(D, S):
    ...
    '''
    import itertools
    c = list(itertools.product(range(len(S)),repeat=len(S)) )
    #c = [(0,0),(0,1),(1,0),(1,1)]
    tmp = {sum([b*a for a,b in list(zip(S,f)) ] ) for f in c}
    return tmp
